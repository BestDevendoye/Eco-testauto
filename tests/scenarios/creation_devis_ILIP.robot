...
Formulaire Devis-Accès-Offre ILIP
    Log To Console    Step-02-Clic sur Indemnité Licenciement Inaptitude professionnelle
...
Tarificateur ILIP
    ...
    Sélectionner la réponse NON à la question l’entreprise couverte au titre du risque incapacité temporaire d’origine professionnelle    ${valeur_champ_entreprise_NON}
    Vérifier la présence du message si la réponse est non_ILIP
    Vérifier si la valeur du champ est NON ILIP      ${valeur_champ_entreprise_NON}
    Sélectionner la réponse OUI à la question l’entreprise couverte au titre du risque incapacité temporaire d’origine professionnelle     ${valeur_champ_entreprise_OUI}
    Vérifier la valeur du champ entreprise est OUI ILIP   ${valeur_champ_entreprise_OUI}
    Sélectionner le type de contrat prévoyance actuel   ${type_contrat_prevoyance_actuel}
    Renseigner le nombre de salariés non cadres  ${valeur_champ_nbr_salaries_ILIP}
    Renseigner la masse salariale annuelle       ${valeur_champ_masse_salariale_ILIP}
    ...
    Vérifier si l'offre sélectionnée est bien affichée dans la partie Résumé    ${offre_selectionnee_ILIP}
    Cliquer sur le bouton Enregistrer
    Set Selenium Speed    0s

Envoi de la proposition commerciale-Offre ILIP
    Log To Console    Step-07-Envoi de la proposition
    Vérifier le titre de la page Synthèse       Indemnités de Licenciement pour Inaptitude Professionnelle        Synthèse de la proposition commerciale et envoi au client
